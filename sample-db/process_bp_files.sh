...
do
    ...
    # ugh, quoting
    # dropped column "Total Size, Mb" because commas
    csvcut -c "Experiment Accession","Experiment Title","Organism Name","Instrument","Submitter","Study Accession","Study Title","Sample Accession","Sample Title","Total RUNs","Total Spots","Total Bases","Library Name","Library Strategy","Library Source","Library Selection" $srares | csvsql --db sqlite:///$rundb --insert --tables sra_result $create
    echo "SRA Result Row Count:" $srares $(sqlite3 $rundb "select count(*) from sra_result")
done

keep=Assay_Type_s,AssemblyName_s,BioProject_s,BioSample_s,Center_Name_s,Consent_s,g1k_analysis_group_s,g1k_pop_code_s,InsertSize_l,Library_Name_s,LoadDate_s,MBases_l,MBytes_l,Platform_s,ReleaseDate_s,Run_s,Sample_Name_s,source_s,SRA_Sample_s,SRA_Study_s
#SampleModel_s dropped because 08 doesn't have it
